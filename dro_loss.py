...
class LossComputer:
    def __init__(self,
        ...
        self.adv_probs = torch.ones(self.n_groups).to(
            self.device)/self.n_groups
        self.exp_avg_loss = torch.zeros(self.n_groups).to(self.device)
        self.exp_avg_initialized = torch.zeros(
            self.n_groups).byte().to(self.device)
    ...
    def loss(self, yhat, y, group_idx=None, is_training=False):
        ...
        # update stats
        self.update_stats(actual_loss, group_loss,
                          group_acc, group_count, weights)
    ...
    def compute_robust_loss(self, group_loss, group_count):
        ...
        self.adv_probs = self.adv_probs * \
            torch.exp(self.step_size*adjusted_loss.data)
    ...
    def compute_robust_loss_btl(self, group_loss, group_count):
        adjusted_loss = self.exp_avg_loss + \
            self.adj/torch.sqrt(self.group_counts)
    ...
    def compute_robust_loss_greedy(self, group_loss, ref_loss):
        ...
        weights = sorted_frac*self.min_var_weight + \
            weights*(1-self.min_var_weight)
    ...
    def get_stats(self, model=None, weight_decay=0):
        stats_dict = {}
        ...
        if model is not None:
            ...
            stats_dict = self.get_model_stats(
                model=model, weight_decay=weight_decay, stats_dict=stats_dict)
